"""
Overfit Test with STFT Features (Batch Overfitting Sanity Check)
===============================================================

This test verifies that the model can overfit on a very small dataset using STFT features.
...
Purpose:
- Verify model architecture is correct with STFT features
...
# Training config
CONFIG = {
    'input_dim': 14 * 65,  # 14 channels * (n_fft//2 + 1) for STFT
    'encoder_dim': 256,
    'decoder_dim': 512,
    'joint_dim': 512,
    'vocab_size': None,
...
def compute_stft_features(eeg_signal, config):
    """Compute STFT magnitude spectrogram for all EEG channels."""
    ...
    for ch_idx in range(n_channels):
        ...
        stft_log = np.log(stft_mag + 1e-9)
        stft_specs.append(stft_log)

    # Stack all channels: (n_channels, n_freq_bins, n_time_frames)
    stft_stacked = np.stack(stft_specs, axis=0)
    # Transpose to: (n_time_frames, n_channels, n_freq_bins)
    stft_transposed = stft_stacked.transpose(2, 0, 1)
    # Flatten channels and freq bins: (n_time_frames, n_channels * n_freq_bins)
...
def load_overfit_dataset(config, samples_per_subject=1):
    ...
    overfit_df = pd.concat(overfit_data, ignore_index=True)
    ...
    print("\n[STEP 3] Load EEG signals and compute STFT features...")
    ...
    for idx, row in tqdm(overfit_df.iterrows(), total=len(overfit_df), desc="Processing"):
        ...
        stft_features = compute_stft_features(eeg_signal, config)
        
        data['features'].append(stft_features)
...
def train(model, train_loader, test_loader, tokenizer, config, device):
    """Main training loop dengan fitur Save & Load Best Model."""
    ...
    for epoch in range(config['num_epochs']):
        ...
        # 3. Logika Simpan Model Terbaik (Berdasarkan Test CER)
        if test_cer < best_cer:
            ...
            print(f"  --> [SAVE] Rekor baru! Best CER: {best_cer:.4f}. Model disimpan.")

    # 4. KRUSIAL: Load kembali model terbaik sebelum keluar dari fungsi train
    if os.path.exists(best_model_path):
        print(f"\n[FINAL] Mengambil model terbaik dari {best_model_path} untuk prediksi akhir...")
        checkpoint = torch.load(best_model_path, weights_only=False)
        model.load_state_dict(checkpoint['model_state_dict'])
        print(f"        (Model berasal dari Epoch {checkpoint['epoch']} dengan CER {checkpoint['cer']:.4f})")
...
def main():
    print("\n" + "=" * 80)
    print("OVERFIT TEST WITH STFT FEATURES - Sanity Check")
    print("=" * 80)
    
    # Load small dataset
    data = load_overfit_dataset(CONFIG, samples_per_subject=2)
    ...
    print("\n" + "=" * 80)
    print("✓ OVERFIT TEST WITH STFT FEATURES COMPLETED")
    print("=" * 80)
