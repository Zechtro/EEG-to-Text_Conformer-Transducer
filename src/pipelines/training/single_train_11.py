...
def load_and_preprocess_dataset(config, target_subject):
    ...
    if len(df) == 0: raise ValueError(f"Tidak ada data ditemukan untuk subject: {target_subject}")
...
def train(model, train_loader, val_loader, tokenizer, config, device, target_subject):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, f'{target_subject}_hilbert_best_model_11_0_IndoGPT.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder, target_subject):
    ...
    csv_path = os.path.join(output_dir, f'{target_subject}_hilbert_test_predictions_11_0_IndoGPT.csv')
...
def plot_training_history(history, output_dir, target_subject):
    ...
    axes[0].set_title(f'Loss History ({target_subject} - IndoGPT)')
    ...
    axes[1].set_title(f'CER History ({target_subject} - IndoGPT)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{target_subject}_hilbert_training_history_11_0_IndoGPT.png'), dpi=300)
    plt.close()
...
def main():
    ...
    print("=" * 80)
    print(f"EEG-to-Text Training Pipeline (Subject: {TARGET_SUBJECT} | IndoGPT Decoder)")
    ...
    with open(os.path.join(OUTPUT_DIR, f'{TARGET_SUBJECT}_hilbert_training_history_11_0_IndoGPT.json'), 'w') as f:
