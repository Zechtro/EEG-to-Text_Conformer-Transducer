...
def load_eeg_signal(id_val, subject, gender):
    ...
    try:
        ...
        return extract_eeg_channels(df)
...
def compute_stft_features(eeg_signal, config):
    ...
    for ch_idx in range(n_channels):
        ...
        stft = librosa.stft(y=signal, n_fft=config['n_fft'], hop_length=config['hop_length'])
...
def load_and_preprocess_dataset(config):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
        ...
        data[split]['features'].append(stft_features)
...
def train(model, train_loader, val_loader, tokenizer, config, device):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, 'all_stft_best_model.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, 'all_stft_test_predictions_0.csv')
...
def main():
    ...
    with open(os.path.join(OUTPUT_DIR, 'all_stft_training_history_0.json'), 'w') as f:
