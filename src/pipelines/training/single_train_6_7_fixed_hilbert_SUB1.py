...
TRAIN_CSV = os.path.join(PROJECT_ROOT, 'dataset/SUB1_eq_3_0_train.csv')
VAL_CSV = os.path.join(PROJECT_ROOT, 'dataset/SUB1_eq_3_0_val.csv')
TEST_CSV = os.path.join(PROJECT_ROOT, 'dataset/SUB1_eq_3_0_test.csv')
...
def preprocess_eeg_advanced(signal, ch_names, config):
    ...
    try:
        ...
        # PERBAIKAN: Hapus awalan 'EEG.' agar dikenali oleh peta 10-20
        ...
        if config.get('remove_eye_artifacts', True):
            ...
            for idx, (label, prob) in enumerate(zip(labels, probabilities)):
                if label in ['eye', 'muscle'] and prob >= config.get('ica_threshold', 0.7):
        ...
        print(f"[WARNING] Preprocessing lanjutan gagal, fallback ke basic CAR: {e}")
        avg_reference = np.mean(signal, axis=1, keepdims=True)
        signal = signal - avg_reference
...
def load_eeg_signal(id_val, subject, gender, config):
    ...
    try:
        ...
        if signal is not None:
            signal = preprocess_eeg_advanced(signal, EEG_CHANNELS, config)
...
def compute_hilbert_spectrum(eeg_signal, config):
    ...
    f_min, f_max = config['f_min'], config['f_max']
    n_bins, hop_length, win_length = config['n_freq_bins'], config['hop_length'], config['win_length']
...
def load_and_preprocess_dataset(config):
    ...
    df_train, df_val, df_test = pd.read_csv(TRAIN_CSV), pd.read_csv(VAL_CSV), pd.read_csv(TEST_CSV)
...
def train_epoch(model, train_loader, optimizer, tokenizer, device, beam_decoder=None):
    ...
    for batch in tqdm(train_loader, desc="Training"):
        ...
        if hasattr(model, 'decoder'):
            model.decoder.train()
            if hasattr(model.decoder, 'lstm'): model.decoder.lstm.train()
            elif hasattr(model.decoder, 'rnn'): model.decoder.rnn.train()
...
def train(model, train_loader, val_loader, tokenizer, config, device):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_final_icalabel_best_model.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, f'{SUBJECT}_eq_3_0_final_icalabel_test_predictions.csv')
...
def plot_training_history(history, output_dir):
    ...
    axes[0].set_title(f'Loss ({SUBJECT} - Final SOTA Preprocessing)')
    ...
    axes[1].set_title(f'CER ({SUBJECT} - Final SOTA Preprocessing)')
...
def main():
    print("=" * 80)
    print(f"EEG-to-Text Training Pipeline ({SUBJECT} | Final SOTA Preprocessing)")
    ...
    tokenizer_path = os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_final_icalabel_char_tokenizer.pkl')
    ...
    with open(os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_final_icalabel_training_history.json'), 'w') as f:
