...
Sesuai dengan eksperimen: single_train_6_1_fixed_hilbert_all.py
HANYA UNTUK SUBJEK: 'all' (Single Execution)
"""
...
TOKENIZER_PATH = os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_fixed_hilbert_char_tokenizer_1_1.pkl')
BEST_MODEL_PATH = os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_fixed_hilbert_best_model_1_1.pt')
OUTPUT_CSV_NAME = f'NOISE_BASELINE_{SUBJECT}_eq_3_0_fixed_hilbert_test_predictions_1_1.csv'
...
def predict_on_noise_and_save(model, test_loader, tokenizer, output_dir, device, beam_decoder, output_csv_name):
    ...
    print(f"\n[SAVE] Prediksi Noise-Baseline disimpan ke: {output_csv_name}")
    print(f"Rata-rata CER pada Noise murni: {predictions_df['cer'].mean():.4f}")
    
    return predictions_df

# ============================================================================
# MAIN EXECUTOR DENGAN PRE-FLIGHT CHECK
# ============================================================================
...
def main():
    ...
    # PRE-FLIGHT CHECK
    # ------------------------------------------------------------------------
    print("\n[STEP 0] PRE-FLIGHT CHECK: Memeriksa kelengkapan file...")
    missing = []
    if not os.path.exists(TEST_CSV): missing.append(f"Test CSV: {os.path.basename(TEST_CSV)}")
    if not os.path.exists(TOKENIZER_PATH): missing.append(f"Tokenizer: {os.path.basename(TOKENIZER_PATH)}")
    if not os.path.exists(BEST_MODEL_PATH): missing.append(f"Bobot Model: {os.path.basename(BEST_MODEL_PATH)}")
    
    if missing:
        print(f"❌ [ERROR] Proses dihentikan. File berikut tidak ditemukan:")
        for m in missing:
            print(f"   - {m}")
        return
        
    print("✓ Seluruh file prasyarat tersedia.")
    ...
    print(f"✓ Bobot model berhasil dimuat dari: {os.path.basename(BEST_MODEL_PATH)}")
