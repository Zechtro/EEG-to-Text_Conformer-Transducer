...
TRAIN_CSV = os.path.join(PROJECT_ROOT, 'dataset/all_eq_3_0_train.csv')
...
def train(model, train_loader, val_loader, tokenizer, config, device):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, f'all_eq_3_0_fixed_hilbert_best_model_1_1.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, f'all_eq_3_0_fixed_hilbert_test_predictions_1_1.csv')
...
def plot_training_history(history, output_dir):
    ...
    axes[0].set_title('Loss History (All Subjects - Hilbert)')
    ...
    axes[1].set_title('CER History (All Subjects - Hilbert)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'all_eq_3_0_fixed_hilbert_training_history_1_1.png'), dpi=300)
    plt.close()
...
def main():
    print("=" * 80)
    print(f"EEG-to-Text Training Pipeline (ALL SUBJECTS | Hilbert Spectrum Features)")
    ...
    tokenizer_path = os.path.join(OUTPUT_DIR, f'all_eq_3_0_fixed_hilbert_char_tokenizer_1_1.pkl')
    ...
    with open(os.path.join(OUTPUT_DIR, f'all_eq_3_0_fixed_hilbert_training_history_1_1.json'), 'w') as f:
    ...
    print("\n" + "=" * 80)
    print("✓ FULL TRAINING PIPELINE COMPLETED FOR ALL SUBJECTS")
    print("=" * 80)
