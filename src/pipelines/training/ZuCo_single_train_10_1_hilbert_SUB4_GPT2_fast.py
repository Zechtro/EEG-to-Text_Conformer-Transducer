...
from misc.beam_decoder import BeamDecoderIndoGPT   # same interface, works for any GPT2Model
...
class ConformerGPT2Transducer(nn.Module):
    """Conformer encoder + GPT-2 decoder (last N blocks unfrozen) + RNN-T joint network."""
    def __init__(self, config):
        super().__init__()
        self.encoder = Conformer(input_dim=config['input_dim'], d_model=config['encoder_dim'])
        self.decoder = IndoGPTDecoder(model_name_or_path='gpt2', joint_dim=config['decoder_dim'])
        ...
        # Unfreeze the last N transformer blocks + final layer norm so GPT-2 can
        # adapt its representations to the EEG encoder output space.
        # (All blocks are frozen by IndoGPTDecoder.__init__ by default.)
        n_unfreeze = config.get('unfreeze_gpt2_blocks', 4)
        if n_unfreeze > 0:
            gpt = self.decoder.gpt
            blocks = gpt.h if hasattr(gpt, 'h') else gpt.transformer.h
            ln_f = gpt.ln_f if hasattr(gpt, 'ln_f') else gpt.transformer.ln_f
...
CONFIG = {
    'input_dim': 14 * 65,   # 14 channels × 65 freq bins = 910

    # GPT-2 small: n_embd=768, same dims as IndoGPT
    'encoder_dim': 356,
    'decoder_dim': 768,
    'joint_dim': 768,
    'vocab_size': None,   # set in main() after tokenizer loaded

    # batch_size=4: prevents int32 overflow in torchaudio rnnt_loss CUDA kernel
    # (B × max_T × (max_U+1) × V must stay < 2^31; GPT-2 V=50258 is large)
    'batch_size': 4,
    'num_epochs': 1500,
    'learning_rate': 1e-4,
    'weight_decay': 1e-3,
    ...
    # GPT-2 fully frozen (0 blocks unfrozen) — experiment #6
    'unfreeze_gpt2_blocks': 0,
    'lr_gpt2': 1e-5,
...
MAX_FEAT_LEN = 400   # enc_out ≤ 100 frames; keeps B×T×(U+1)×V < 2^31

def get_enc_out_len(feat_len):
    """Mirrors ConformerGPT2Transducer.get_encoder_out_lengths — no model needed."""
    out = (feat_len - 1) // 2 + 1
    out = (out - 1) // 2 + 1
    return out

def filter_split(feats, tgts, meta, tokenizer, batch_size=4, vocab_size=50258):
    """Drop samples that would cause int32 overflow in torchaudio rnnt_loss CUDA kernel.

    GPT-2 vocab (50258) is large; the kernel indexes B×max_T×max_U×V with int32.
    Also drops empty targets and enc_out < tgt_len cases.
    """
    INT32_MAX = 2 ** 31 - 1
    ...
    if dropped:
        print(f"    Dropped {dropped} samples (enc<tgt / too long / int32 overflow risk)")
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # GPT2Tokenizer.encode returns IDs without special tokens by default
        # Shift +1 to reserve index 0 for RNN-T <blank>
...
def train(model, train_loader, val_loader, config, device):
    # Differential LR: encoder/joiner at learning_rate, unfrozen GPT-2 at lr_gpt2 (10× lower)
    gpt2_ids = set(id(p) for p in model.decoder.gpt.parameters() if p.requires_grad)
    gpt2_params = [p for p in model.parameters() if p.requires_grad and id(p) in gpt2_ids]
    other_params = [p for p in model.parameters() if p.requires_grad and id(p) not in gpt2_ids]
    optimizer = optim.Adam(
        [{'params': other_params, 'lr': config['learning_rate']},
         {'params': gpt2_params,  'lr': config.get('lr_gpt2', config['learning_rate'] * 0.1)}],
        weight_decay=config['weight_decay']
    )
    history = {'train_loss': [], 'val_loss': []}

    model_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_GPT2_fast_model_10_1.pt')
...
def predict_and_save_csv(model, train_loader, val_loader, test_loader, tokenizer, device):
    import pandas as pd
    decoder = BeamDecoderIndoGPT(model, tokenizer, beam_size=3, max_sym_per_frame=3)
    ...
    csv_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_GPT2_fast_test_predictions_10_1.csv')
...
def plot_training_history(history):
    ...
    ax.set_title(f'Loss History (ZuCo {SUBJECT_ID} NR — Hilbert GPT-2 Fast)')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_GPT2_fast_training_history_10_1.png')
...
def main():
    print('=' * 80)
    print(f'ZuCo EEG-to-Text Training (FAST) — Subject: {SUBJECT_ID} ({SUBJECT_VERSION}) | NR | Hilbert + GPT-2')
    print(f'[INFO] Device: {DEVICE}')
    print('=' * 80)

    print('\n[STEP 0] Loading GPT2Tokenizer...')
    tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
    tokenizer.pad_token = tokenizer.eos_token   # GPT-2 has no native pad token

    CONFIG['vocab_size'] = tokenizer.vocab_size + 1   # +1 for <blank> at index 0
    print(f'  Vocab size (including blank): {CONFIG["vocab_size"]}')

    data = load_and_preprocess_dataset(CONFIG)

    print('\n[STEP 3b] Filtering unsafe samples (short enc, too long, int32 overflow risk)...')
    bs, vs = CONFIG['batch_size'], CONFIG['vocab_size']
    for split in ('train', 'val', 'test'):
        f, t, m = filter_split(data[split]['features'], data[split]['targets'],
                               data[split]['metadata'], tokenizer,
                               batch_size=bs, vocab_size=vs)
        data[split]['features'], data[split]['targets'], data[split]['metadata'] = f, t, m
    print(f"  After filter — Train: {len(data['train']['features'])} | "
          f"Val: {len(data['val']['features'])} | Test: {len(data['test']['features'])}")
    ...
    model = ConformerGPT2Transducer(CONFIG).to(DEVICE)

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    print(f'\n[INFO] Trainable params: {trainable:,} / {total:,} total')

    history = train(model, train_loader, val_loader, CONFIG, DEVICE)

    hist_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_GPT2_fast_training_history_10_1.json')
