...
def predict_and_save_csv(model, val_loader, test_loader, tokenizer, device):
    ...
    # Val + test cover generalisation; train loss already tracks training.
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_fast_test_predictions_6_1.csv')
...
def plot_training_history(history):
    ...
    ax.set_title('Loss History (ZuCo All Subjects NR — LogMel Fast)')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_fast_training_history_6_1.png')
...
def main():
    print('=' * 80)
    print('ZuCo EEG-to-Text Training (FAST) — All Subjects (v1 + v2) | NR | Log-Mel')
    ...
    tok_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_fast_char_tokenizer_6_1.pkl')
    ...
    hist_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_fast_training_history_6_1.json')
