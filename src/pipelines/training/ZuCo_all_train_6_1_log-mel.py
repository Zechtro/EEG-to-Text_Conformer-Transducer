...
SUBJECT = 'all'   # used as label in filenames

ZUCO_V1_SUBJECTS = ['ZAB', 'ZDM', 'ZDN', 'ZGW', 'ZJM', 'ZJN',
                     'ZJS', 'ZKB', 'ZKH', 'ZKW', 'ZMG', 'ZPH']
ZUCO_V2_SUBJECTS = ['YAC', 'YAG', 'YAK', 'YDG', 'YDR', 'YFR', 'YFS',
                     'YHS', 'YIS', 'YLS', 'YMD', 'YMS', 'YRH',
                     'YRK', 'YRP', 'YSD', 'YSL', 'YTL']
...
CONFIG = {
    ...
    'sample_rate': 500,   # ZuCo BioSemi @ 500 Hz
    'n_fft': 256,
    'win_length': 256,
    'hop_length': 32,
    'n_mels': 64,
...
def load_zuco_subject(subject_id, version):
    """
    Load NR-task sentences + EEG for one subject. Returns ([], []) if file missing.
    ...
    if not os.path.isfile(filepath):
        osf_id = 'q3zws' if version == 'v1' else '2urht'
        task_folder = 'task2 - NR' if version == 'v1' else 'task1 - NR'
        print(f'  [SKIP] {subject_id} ({version}) — file not found: {filepath}')
        print(f'         Download: osf -p {osf_id} fetch '
              f'"osfstorage/{task_folder}/Matlab files/results{subject_id}_NR.mat" '
              f'{filepath}')
        return [], []

    print(f'  Loading: {subject_id} ({version}) — {os.path.basename(filepath)}')
    try:
...
def load_all_zuco_subjects():
    """
    Load NR EEG data for all ZuCo subjects (v1 + v2).
    Returns parallel lists: all_sentences, all_eeg (14-ch selected), all_meta.
    """
    ...
    for subject_id, version in (
        ...
        for i, (sent, eeg) in enumerate(zip(sents, eegs)):
            ...
            all_eeg.append(eeg[:, EMOTIV_CHANNEL_INDICES])   # (time, 14)
...
def load_and_preprocess_dataset(config):
    print(f'\n[STEP 1] Loading ZuCo NR data — all subjects (v1 + v2)...')
    all_sentences, all_eeg, all_meta = load_all_zuco_subjects()

    if not all_sentences:
        raise RuntimeError(
            'No data loaded. Ensure ZuCo mat files are in:\n'
            f'  {ZUCO_DATA_PATH}/v1/NR/  and  {ZUCO_DATA_PATH}/v2/NR/'
        )
    print(f'\n  Total sentences across all subjects: {len(all_sentences)}')

    print(f'\n[STEP 2] Global 70/10/20 split (seed={config["random_seed"]})...')
...
def train(model, train_loader, val_loader, tokenizer, config, device):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_best_model_6_1.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, device, beam_decoder):
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_test_predictions_6_1.csv')
...
def plot_training_history(history):
    ...
    axes[0].set_title('Loss History (ZuCo All Subjects NR — LogMel)')
    axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('Loss')
    axes[0].legend(); axes[0].grid(True)

    axes[1].plot(epochs, history['val_cer'], 'r-s', label='Val CER')
    axes[1].set_title('CER History (ZuCo All Subjects NR — LogMel)')
    axes[1].set_xlabel('Epoch'); axes[1].set_ylabel('CER')
    axes[1].legend(); axes[1].grid(True)

    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_training_history_6_1.png')
...
def main():
    print('=' * 80)
    print('ZuCo EEG-to-Text Training — All Subjects (v1 + v2) | NR | Log-Mel')
    ...
    tok_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_char_tokenizer_6_1.pkl')
    ...
    hist_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_log-mel_training_history_6_1.json')
    ...
    print('\n' + '=' * 80)
    print('ZuCo TRAINING COMPLETE — All Subjects (v1 + v2) | NR')
