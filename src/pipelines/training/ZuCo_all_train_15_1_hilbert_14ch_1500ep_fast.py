...
CACHE_PATH = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_features_cache.pkl')
...
def load_zuco_subject(subject_id, version):
    ...
    if not os.path.isfile(filepath):
        ...
        return [], []
    print(f'  Loading: {subject_id} ({version})')
    try:
    ...
    for ch_idx in range(n_channels):
        ...
        if current_n_samples < win_length:
            ...
            for t in range(n_frames):
                start = t * hop_length
                framed_spec[:, t] = np.mean(hilbert_spec[:, start:start + win_length], axis=1)
...
def load_and_preprocess_dataset(config):
    ...
    if not all_sentences:
        raise RuntimeError(f'No data loaded. Check {ZUCO_DATA_PATH}/v1/NR/ and v2/NR/')
    ...
    if os.path.exists(CACHE_PATH):
        print(f'\n[STEP 3] Loading cached features from:\n  {CACHE_PATH}')
        with open(CACHE_PATH, 'rb') as f:
            cached = pickle.load(f)
        all_feats = cached['features']
        print(f'  Loaded {len(all_feats)} cached feature arrays')
    else:
        print(f'\n[STEP 3] Extracting Hilbert features for ALL sentences '
              f'({N_EXTRACT_JOBS} parallel workers)...')
        print('  NOTE: CEEMDAN over ~9k sentences may take several hours.')
        args_list = [
            (all_eeg[i], all_sentences[i], all_meta[i], config)
            for i in range(len(all_sentences))
        ]
        all_feats = [None] * len(all_sentences)
        with multiprocessing.Pool(processes=N_EXTRACT_JOBS) as pool:
            for orig_idx, result in tqdm(
                    enumerate(pool.imap(_extract_sentence, args_list)),
                    total=len(args_list), desc='  Extracting'):
                all_feats[orig_idx] = result  # None if too short

        print(f'  Saving cache to:\n  {CACHE_PATH}')
        with open(CACHE_PATH, 'wb') as f:
            pickle.dump({'features': all_feats}, f)
        print('  Cache saved.')
...
def train(model, train_loader, val_loader, config, device):
    ...
    model_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_model_15_1.pt')
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc):
            ...
            for i, meta in enumerate(batch['metadata']):
                ...
                rows.append({'sentence_idx': meta['sentence_idx'],
                             'subject': meta['subject'], 'version': meta['version'],
                             'sentence': gt, 'prediction': pred,
                             'cer': compute_cer(gt, pred)})
    return rows

def predict_and_save_csv(model, val_loader, test_loader, tokenizer, device):
    import pandas as pd
    decoder = beam_decoder_char.BeamDecoderChar(model, tokenizer,
                                                beam_size=3, max_sym_per_frame=15)
    print('\n[STEP 6] Computing CER on val + test (train skip — terlalu besar)...')
    ...
    print(f'\n  Average Val  CER: {val_cer:.4f}')
    print(f'  Average Test CER: {test_cer:.4f}')

    csv_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_test_predictions_15_1.csv')
    pd.DataFrame(test_rows).to_csv(csv_path, index=False)
    print(f'[SAVE] {csv_path}')
    return val_cer, test_cer
...
def plot_training_history(history):
    ...
    ax.set_title('Loss — ZuCo All Subjects NR | Hilbert 14ch LSTM 1500ep')
    ax.set_xlabel('Epoch'); ax.set_ylabel('Loss')
    ax.legend(); ax.grid(True); plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_training_history_15_1.png')
    plt.savefig(path, dpi=300); plt.close()
    print(f'[SAVE] {path}')

def main():
    print('=' * 80)
    print('ZuCo Training — All Subjects (v1+v2) | NR | Hilbert 14ch LSTM | 1500 epoch')
    ...
    tok_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_char_tokenizer_15_1.pkl')
    if os.path.exists(tok_path):
        print(f'  Loading existing: {tok_path}')
    ...
    hist_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_14ch_training_history_15_1.json')
    with open(hist_path, 'w') as f:
        json.dump(history, f, indent=2)

    plot_training_history(history)
    val_cer, test_cer = predict_and_save_csv(
        model, val_loader, test_loader, tokenizer, DEVICE)

    print('\n' + '=' * 80)
    print('TRAINING COMPLETE — All Subjects | Hilbert 14ch LSTM 1500ep')
    print(f'  Val CER: {val_cer:.4f} | Test CER: {test_cer:.4f}')
