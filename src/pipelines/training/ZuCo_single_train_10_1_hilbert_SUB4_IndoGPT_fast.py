...
# transformers compatibility patches — MUST run before importing indobenchmark
...
CONFIG = {
    ...
    # (B × max_T × (max_U+1) × V must be < 2^31; IndoGPT V=40005 is 800× larger than char-level)
    'batch_size': 4,
    'num_epochs': 1500,
    'learning_rate': 1e-4,   # lower — GPT decoder is sensitive
    'weight_decay': 1e-3,
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Encode without special tokens (BOS/EOS=50256 exceeds vocab), shift +1 for <blank>
        encoded_tokens = self.tokenizer.encode(self.targets[idx], add_special_tokens=False)
...
def get_enc_out_len(feat_len):
    """Mirrors ConformerIndoGPTTransducer.get_encoder_out_lengths — no model needed."""
    out = (feat_len - 1) // 2 + 1
    out = (out - 1) // 2 + 1
    return out

MAX_FEAT_LEN = 400   # enc_out ≤ 100; keeps B×T×(U+1)×V < 2^31 for any reasonable U

def filter_split(feats, tgts, meta, tokenizer, batch_size=4, vocab_size=40005):
    """Drop samples that would cause int32 overflow in torchaudio rnnt_loss CUDA kernel.

    The kernel indexes with int32 over B × max_T × max_U × V — must stay < 2^31.
    We guard per-sample so the worst-case batch product stays safe.
    Also drops empty targets and samples shorter than their token sequence.
    """
    INT32_MAX = 2 ** 31 - 1
...
def train(model, train_loader, val_loader, config, device):
    # Only train params that require grad — IndoGPT decoder weights are frozen
    ...
    model_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_IndoGPT_fast_model_10_1.pt')
...
def predict_and_save_csv(model, train_loader, val_loader, test_loader, tokenizer, device):
    ...
    csv_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_IndoGPT_fast_test_predictions_10_1.csv')
...
def plot_training_history(history):
    ...
    ax.set_title(f'Loss History (ZuCo {SUBJECT_ID} NR — Hilbert IndoGPT Fast)')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_IndoGPT_fast_training_history_10_1.png')
...
def main():
    print('=' * 80)
    print(f'ZuCo EEG-to-Text Training (FAST) — Subject: {SUBJECT_ID} ({SUBJECT_VERSION}) | NR | Hilbert + IndoGPT')
    print(f'[INFO] Device: {DEVICE}')
    print('=' * 80)

    print('\n[STEP 0] Loading IndoNLGTokenizer (IndoGPT)...')
    tokenizer = IndoNLGTokenizer.from_pretrained('indobenchmark/indogpt')

    # IndoNLGTokenizer.pad() doesn't accept padding_side kwarg from newer transformers
    def _dummy_pad(encoded_inputs, **kwargs):
        return encoded_inputs
    tokenizer.pad = _dummy_pad
    ...
    print('\n[STEP 3b] Filtering unsafe samples (short enc, long seq, int32 overflow risk)...')
    ...
    hist_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_IndoGPT_fast_training_history_10_1.json')
