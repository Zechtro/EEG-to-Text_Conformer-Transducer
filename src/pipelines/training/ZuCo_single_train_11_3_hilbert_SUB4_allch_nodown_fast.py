...
from model import ConformerTransducer as _  # triggers sys.path setup for sub-packages
from conformer_block import ConformerBlock
from LSTM.decoder import LSTMDecoder
from joiner import JointNetwork

class ConformerNoDownsampleEncoder(nn.Module):
    """Conformer encoder with a plain Linear input projection — no temporal
    downsampling. All input time frames are preserved (unlike the default
    ConvSubsampling which reduces time by 4×)."""
    def __init__(self, input_dim, d_model, num_heads=4, num_layers=8,
                 conv_kernel_size=31, dropout=0.1, ffn_expansion_factor=4):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        x = self.proj(x)       # (B, T, d_model) — T unchanged
        x = self.dropout(x)
        for block in self.conformer_blocks:
            x = block(x, mask)
        return x

class ConformerNoDownsampleTransducer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.encoder = ConformerNoDownsampleEncoder(
        ...
        self.decoder = LSTMDecoder(
            ...
            hidden_dim=config['decoder_dim'], num_layers=1,
    ...
    def get_encoder_out_lengths(self, input_lengths):
        return input_lengths  # no downsampling — lengths pass through unchanged
...
CONFIG = {
    ...
    'ica_threshold': 0.8,
    'ica_n_components': 30,  # fewer components for stability & speed with 105 channels
...
# Cache file for extracted features — avoids repeating the multi-hour extraction.
FEATURE_CACHE_PATH = os.path.join(
    OUTPUT_DIR,
    f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_features_cache_11_1.pkl'
)
...
def train(model, train_loader, val_loader, config, device):
    ...
    model_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_fast_model_11_3.pt')
...
def predict_and_save_csv(model, train_loader, val_loader, test_loader, tokenizer, device):
    ...
    csv_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_fast_test_predictions_11_3.csv')
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_fast_training_history_11_3.png')
...
def main():
    ...
    print(f'[INFO] Channels: {N_CHANNELS}  |  input_dim: {CONFIG["input_dim"]}')
    print('=' * 80)
    ...
    tok_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_fast_char_tokenizer_11_3.pkl')
    ...
    model = ConformerNoDownsampleTransducer(CONFIG).to(DEVICE)
    ...
    hist_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_hilbert_allch_fast_training_history_11_3.json')
    ...
    print('\n' + '=' * 80)
    print(f'ZuCo TRAINING COMPLETE (FAST, ALL CHANNELS) — Subject: {SUBJECT_ID} ({SUBJECT_VERSION})')
