...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder, target_subject):
    ...
    csv_path = os.path.join(output_dir, f'{target_subject}_stft_test_predictions_0.csv')
...
def plot_training_history(history, output_dir, target_subject):
    ...
    axes[0].set_title(f'Loss History ({target_subject})')
    ...
    axes[1].set_title(f'CER History ({target_subject})')
