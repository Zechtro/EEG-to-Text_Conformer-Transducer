...
TOKENIZER_PATH  = os.path.join(OUTPUT_DIR, 'ZuCo_ZGW_NR_log-mel_fast_char_tokenizer_6_1.pkl')
MODEL_PATH      = os.path.join(OUTPUT_DIR, 'ZuCo_ZGW_NR_log-mel_fast_model_6_1.pt')
OUTPUT_CSV_NAME = 'ZuCo_NOISE_BASELINE_ZGW_NR_log-mel_fast_test_predictions_6_1.csv'
...
def predict_on_noise(model, test_loader, tokenizer, device, beam_decoder):
    model.eval()
    rows = []
    print('\n' + '=' * 70)
...
def main():
    print('=' * 70)
    print(f'ZuCo Noise-Baseline | ZGW | Log-Mel + Char LSTM (v6_1)')
    ...
    predict_on_noise(model, test_loader, tokenizer, DEVICE, beam_decoder)
