...
def train(model, train_loader, val_loader, tokenizer, config, device, target_subject):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, f'{target_subject}_hilbert_best_model_2_0.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder, target_subject):
    ...
    csv_path = os.path.join(output_dir, f'{target_subject}_hilbert_test_predictions_2_0.csv')
...
def plot_training_history(history, output_dir, target_subject):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{target_subject}_hilbert_training_history_2_0.png'), dpi=300)
    plt.close()
...
def main():
    ...
    with open(os.path.join(OUTPUT_DIR, f'{TARGET_SUBJECT}_hilbert_training_history_2_0.json'), 'w') as f:
