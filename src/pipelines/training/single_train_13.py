...
def train(model, train_loader, val_loader, tokenizer, config, device, target_subject):
    ...
    best_model_path = os.path.join(OUTPUT_DIR, f'{target_subject}_hilbert_best_model_13_0_IndoGPT.pt')
    best_cer = float('inf')
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder, target_subject):
    ...
    csv_path = os.path.join(output_dir, f'{target_subject}_hilbert_test_predictions_13_0_IndoGPT.csv')
...
def plot_training_history(history, output_dir, target_subject):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{target_subject}_hilbert_training_history_13_0_IndoGPT.png'), dpi=300)
    plt.close()
...
def main():
    ...
    with open(os.path.join(OUTPUT_DIR, f'{TARGET_SUBJECT}_hilbert_training_history_13_0_IndoGPT.json'), 'w') as f:
