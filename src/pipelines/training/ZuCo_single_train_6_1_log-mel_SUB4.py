...
SUBJECT_ID = 'ZGW'        # 4th subject in ZuCo v1.0 (alphabetical) → "SUB4"
SUBJECT_VERSION = 'v1'    # 'v1' or 'v2'
...
# Emotiv Epoc X channel names in order (used for ICA frontal-channel lookup).
EMOTIV_CHANNELS = ['AF3', 'F7', 'F3', 'FC5', 'T7', 'P7', 'O1', 'O2',
                   'P8', 'T8', 'FC6', 'F4', 'F8', 'AF4']

# 0-based indices of the 14 Emotiv-equivalent channels within ZuCo's 105-ch array.
# ZuCo rawData = FullEEG.data[0:105,:] where FullEEG follows BioSemi 128-ch
# A/B/C/D block ordering (A1..A32 = indices 0-31, B1..B32 = indices 32-63, ...).
# A-block:  Fp1 AF7 AF3 F1 F3 F5 F7 FT7 FC5 FC3 FC1 C1 C3 C5 T7 TP7 CP5 CP3
#           CP1 P1 P3 P5 P7 P9 PO7 PO3 O1 Iz Oz POz Pz CPz
# B-block:  Fp2 AF8 AF4 AFz Fz F2 F4 F6 F8 FT8 FC6 FC4 FC2 FCz Cz C2 C4 C6
...
CONFIG = {
    'input_dim': 14 * 64,      # 14 channels × 64 mel bins
    'encoder_dim': 128,
    'decoder_dim': 128,
    'joint_dim': 128,
    'vocab_size': None,
    ...
    # ZuCo EEG sample rate is 500 Hz; window / hop scaled accordingly
    'sample_rate': 500,
    'n_fft': 256,       # 0.5-second window at 500 Hz
    'win_length': 256,
    'hop_length': 32,   # ~62.5 ms stride
    'n_mels': 64,
    'f_min': 0.5,
    'f_max': 45.0,
...
def normalize_text(text):
    """
    Clean sentence text for character-level EEG-to-text training.
    Steps:
      1. Lowercase
      2. Replace hyphens / en-dashes / em-dashes with a space
      3. Remove all remaining punctuation (keep a-z, 0-9, space)
      4. Collapse multiple spaces
    Numbers and years are kept as digits — the EEG captures the visual
    reading of what was on screen, so the digit string is the correct target.
    """
    text = text.lower()
    text = re.sub(r'[-–—]', ' ', text)           # hyphens/dashes → space
    text = re.sub(r"[^a-z0-9\s]", '', text)                # remove all other punctuation
    text = re.sub(r'\s+', ' ', text).strip()
    return text

def load_zuco_sentences_scipy(filepath):
    """
    Load from ZuCo v1 mat files (MATLAB < 7.3, scipy-readable).
    rawData shape in scipy: (105, time) — must transpose to (time, 105).
    """
    mat = sio.loadmat(filepath, squeeze_me=True, struct_as_record=False)
    data = mat.get('sentenceData', mat.get('data', None))
    if data is None:
        raise KeyError(
            f"No 'sentenceData' key in {filepath}. "
            f"Found keys: {[k for k in mat.keys() if not k.startswith('_')]}"
        )
    ...
    for trial in (data if hasattr(data, '__iter__') else [data]):
        ...
        eeg_arrays.append(raw.T.astype(np.float32))   # (105, time) -> (time, 105)
...
def load_zuco_sentences_h5py(filepath):
    """
    Load from ZuCo v2 mat files (MATLAB v7.3 / HDF5, h5py-readable).
    In HDF5 format, arrays are row-major (C order), so what MATLAB stored as
    (105, time) comes out as (time, 105) — no transpose needed.
    """
    import h5py
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        for i in range(n_sents):
            ...
            if raw.ndim != 2:
                continue
            # h5py reads MATLAB arrays transposed: (105,time) in MATLAB → (time,105) here
            if raw.shape[1] == 105:
                pass                    # already (time, 105)
            elif raw.shape[0] == 105:
                raw = raw.T             # unexpected: (105, time) → (time, 105)
            else:
                continue
...
def load_zuco_subject(subject_id, version):
    """
    Load NR-task data for one ZuCo subject.
    v1 files are MATLAB < 7.3 (scipy); v2 files are MATLAB v7.3 HDF5 (h5py).
    Returns: (sentences, eeg_arrays)
    """
    nr_dir = os.path.join(ZUCO_DATA_PATH, version, 'NR')
    filepath = os.path.join(nr_dir, f'results{subject_id}_NR.mat')
    if not os.path.isfile(filepath):
        raise FileNotFoundError(
            f"ZuCo mat file not found: {filepath}\n"
            f"Download with:\n"
            f"  osf -p {'q3zws' if version=='v1' else '2urht'} "
            f"fetch \"osfstorage/{'task2' if version=='v1' else 'task1'} - NR/Matlab files/"
            f"results{subject_id}_NR.mat\" {filepath}"
        )
...
def compute_logmel_spectrogram(eeg_signal, config):
    """EEG (time, channels) → log-mel features (frames, channels*n_mels)."""
    signal_tensor = torch.FloatTensor(eeg_signal.T)   # (channels, time)
    ...
    mel_spec = mel_transform(signal_tensor)            # (channels, n_mels, n_frames)
    log_mel = db_transform(mel_spec).numpy()
    log_mel = log_mel.transpose(2, 0, 1)               # (n_frames, channels, n_mels)
...
def load_and_preprocess_dataset(config):
    ...
    # Use hardcoded indices for the 14 Emotiv-equivalent channels in ZuCo's 105-ch layout
    ...
    for split_name, indices in [('train', idx_train), ('val', idx_val), ('test', idx_test)]:
        ...
        for i in tqdm(indices, desc=f"  {split_name}"):
            eeg = eeg_arrays[i][:, emotiv_indices]   # select 14 channels
            ...
            if config.get('remove_eye_artifacts', True):
                eeg = remove_ocular_artifacts_ica(eeg, EMOTIV_CHANNELS,
                                                  config['ica_threshold'])
    ...
    for batch in tqdm(train_loader, desc='Training'):
        ...
        if beam_decoder is not None:
            for i in range(features.shape[0]):
                pred = beam_decoder.decode(features[i:i+1])
                ref = tokenizer.int_to_text(targets[i].cpu().numpy().tolist())
                total_cer += compute_cer(ref, pred)

        total_loss += loss.item()
        num_batches += 1
        count += len(targets)

    cer_out = total_cer / count if (count and beam_decoder is not None) else float('nan')
    return (total_loss / num_batches if num_batches else 0, cer_out)
...
def evaluate(model, loader, tokenizer, device, beam_decoder=None, desc='Evaluating'):
    ...
    return (total_loss / len(loader) if len(loader) else 0,
            total_cer / count if count else 1.0)

def train(model, train_loader, val_loader, tokenizer, config, device):
    optimizer = optim.Adam(model.parameters(),
                           lr=config['learning_rate'], weight_decay=config['weight_decay'])
    beam_decoder = beam_decoder_char.BeamDecoderChar(model, tokenizer,
                                                     beam_size=3, max_sym_per_frame=15)
    history = {'train_loss': [], 'val_loss': [], 'val_cer': []}

    best_model_path = os.path.join(
        OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_log-mel_best_model_6_1.pt')
    best_cer = float('inf')
    ...
    for epoch in range(config['num_epochs']):
        ...
        train_loss, _ = train_epoch(
            model, train_loader, optimizer, tokenizer, device, None)
        val_loss, val_cer = evaluate(
            model, val_loader, tokenizer, device, beam_decoder, desc='Validating')
        ...
        if val_cer < best_cer:
            ...
            print(f'  --> [SAVE] New best val CER: {best_cer:.4f}')
...
def predict_and_save_csv(model, test_loader, tokenizer, device, beam_decoder):
    model.eval()
    rows = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc='Testing'):
            ...
            for i, meta in enumerate(batch['metadata']):
                gt = meta['sentence']
                pred = beam_decoder.decode(features[i:i+1]) if beam_decoder else ''
                rows.append({
    ...
    csv_path = os.path.join(OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_log-mel_test_predictions_6_1.csv')
    df.to_csv(csv_path, index=False)
    print(f'[SAVE] Predictions → {csv_path}')
    print(f'Average Test CER: {df["cer"].mean():.4f}')
    return df
...
def plot_training_history(history):
    ...
    axes[0].set_title(f'Loss History (ZuCo {SUBJECT_ID} NR — LogMel)')
    axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('Loss')
    axes[0].legend(); axes[0].grid(True)

    axes[1].plot(epochs, history['val_cer'], 'r-s', label='Val CER')
    axes[1].set_title(f'CER History (ZuCo {SUBJECT_ID} NR — LogMel)')
    axes[1].set_xlabel('Epoch'); axes[1].set_ylabel('CER')
    axes[1].legend(); axes[1].grid(True)

    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_log-mel_training_history_6_1.png')
...
def main():
    print('=' * 80)
    print(f'ZuCo EEG-to-Text Training — Single Subject: {SUBJECT_ID} ({SUBJECT_VERSION}) | NR | Log-Mel')
    ...
    tok_path = os.path.join(OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_log-mel_char_tokenizer_6_1.pkl')
    ...
    hist_path = os.path.join(OUTPUT_DIR, f'ZuCo_{SUBJECT_ID}_NR_log-mel_training_history_6_1.json')
    with open(hist_path, 'w') as f:
        json.dump(history, f, indent=2)

    plot_training_history(history)
    predict_and_save_csv(model, test_loader, tokenizer, DEVICE, beam_decoder)

    print('\n' + '=' * 80)
    print(f'ZuCo TRAINING COMPLETE — Subject: {SUBJECT_ID} ({SUBJECT_VERSION})')
