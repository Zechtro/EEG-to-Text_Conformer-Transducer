...
MODEL_PATH      = os.path.join(OUTPUT_DIR, 'ZuCo_ZGW_NR_hilbert_GPT2_fast_model_10_1.pt')
OUTPUT_CSV_NAME = 'ZuCo_NOISE_BASELINE_ZGW_NR_hilbert_GPT2_fast_test_predictions_10_1.csv'
...
class ConformerGPT2Transducer(nn.Module):
    def __init__(self, config):
        ...
        if n_unfreeze > 0:
            ...
            for p in ln_f.parameters(): p.requires_grad = True
...
def compute_hilbert_spectrum(eeg_signal, config):
    ...
    fs = config['sample_rate']; f_min = config['f_min']; f_max = config['f_max']
    n_bins = config['n_freq_bins']; hop_length = config['hop_length']
    win_length = config['win_length']; start_imf = config.get('start_imf', 2)
    ...
    for ch_idx in range(n_channels):
        ...
        current_n = n_samples
        hspec = np.zeros((n_bins, n_samples))
        for imf in imfs:
            ...
            for t in range(n_samples):
                b = bins[t]
                if 0 <= b < n_bins: hspec[b, t] += amp[t] ** 2
        if current_n > win_length:
            remainder = (current_n - win_length) % hop_length
            if remainder > 0:
                hspec = np.pad(hspec, ((0, 0), (0, hop_length - remainder)))
                current_n += hop_length - remainder
        if current_n < win_length:
            ...
            for t_idx in range(n_frames):
                ...
                framed[:, t_idx] = np.mean(hspec[:, start:start + win_length], axis=1)
        all_spectra.append(framed)
    all_spectra = np.array(all_spectra)
    features = all_spectra.transpose(2, 0, 1).reshape(all_spectra.shape[2], -1)
...
def extract_features(eeg_arrays, sentences, indices, config):
    ...
    for i in tqdm(indices, desc='  Extracting Hilbert Spectrum'):
        ...
        if eeg.shape[0] < config['hop_length']: continue
...
def main():
    print('=' * 70)
    print(f'ZuCo Noise-Baseline | ZGW | Hilbert + GPT-2 Frozen (v10_1)')
    ...
    beam_decoder = BeamDecoderIndoGPT(model, tokenizer, beam_size=3, max_sym_per_frame=3)
