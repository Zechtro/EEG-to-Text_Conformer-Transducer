...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, 'all_subjects_hilbert_test_predictions_2_IndoGPT.csv')
...
def plot_training_history(history, output_dir):
    ...
    axes[0].set_title('Loss History (All Subjects - IndoGPT)')
    ...
    axes[1].set_title('CER History (All Subjects - IndoGPT)')
...
def main():
    ...
    # Tambahkan drop_last=True untuk training
    train_loader = DataLoader(train_dataset, batch_size=CONFIG['batch_size'], shuffle=True, collate_fn=collate_batch, drop_last=True)
