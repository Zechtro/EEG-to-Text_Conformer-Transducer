...
SUBJECT = 'all'  # Ganti subjek di sini
...
def load_eeg_signal(id_val, subject, gender, config):
    ...
    # PERBAIKAN: Gunakan str(id_val) mencegah TypeError Pandas
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, f'{SUBJECT}_eq_3_0_hilbert_test_predictions_2_1_IndoGPT.csv')
...
def plot_training_history(history, output_dir):
    ...
    axes[0].set_title(f'Loss History ({SUBJECT} - IndoGPT)')
    ...
    axes[1].set_title(f'CER History ({SUBJECT} - IndoGPT)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{SUBJECT}_eq_3_0_hilbert_training_history_2_1_IndoGPT.png'), dpi=300)
    plt.close()
...
def main():
    print("=" * 80)
    print(f"EEG-to-Text Training Pipeline (All Subjects | IndoGPT Decoder)")
    ...
    with open(os.path.join(OUTPUT_DIR, f'{SUBJECT}_eq_3_0_hilbert_training_history_2_1_IndoGPT.json'), 'w') as f:
