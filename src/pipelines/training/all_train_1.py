...
CONFIG = {
    'input_dim': 14 * 65,  # 14 channel * 65 pita frekuensi
    'encoder_dim': 128,
    'decoder_dim': 128,
    'joint_dim': 128,
    'vocab_size': None,
    
    'batch_size': 16,       # Pastikan VRAM Anda cukup untuk batch dari semua subjek
    'num_epochs': 200, 
    'learning_rate': 1e-3,
    'weight_decay': 1e-4,  
...
def split_dataset_by_sentence(df, train_ratio=0.7, val_ratio=0.1, test_ratio=0.2, seed=42):
    ...
    Because df contains ALL subjects, assigning a sentence label to 'train'
    ensures EVERY subject's recording of that sentence goes to 'train'.
    """
    np.random.seed(seed)
...
def predict_and_save_csv(model, test_loader, tokenizer, output_dir, device, beam_decoder):
    ...
    csv_path = os.path.join(output_dir, 'all_subjects_hilbert_test_predictions_1_0.csv')
...
def plot_training_history(history, output_dir):
    ...
    axes[0].set_title('Loss History (All Subjects - Hilbert Spectrum)')
    ...
    axes[1].set_title('CER History (All Subjects - Hilbert Spectrum)')
...
def main():
    ...
    with open(os.path.join(OUTPUT_DIR, 'all_subjects_hilbert_training_history_1_0.json'), 'w') as f:
