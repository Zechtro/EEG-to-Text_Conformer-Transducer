...
def train(model, train_loader, val_loader, config, device):
    ...
    model_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_fast_model_6_1.pt')
...
def predict_and_save_csv(model, val_loader, test_loader, tokenizer, device):
    ...
    print('\n[STEP 6] Computing CER on val + test splits...')
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_fast_test_predictions_6_1.csv')
...
def plot_training_history(history):
    ...
    ax.set_title('Loss History (ZuCo All Subjects NR — Hilbert Fast)')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_fast_training_history_6_1.png')
...
def main():
    print('=' * 80)
    print('ZuCo EEG-to-Text Training (FAST) — All Subjects (v1 + v2) | NR | Hilbert')
    ...
    tok_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_fast_char_tokenizer_6_1.pkl')
    ...
    hist_path = os.path.join(OUTPUT_DIR, 'ZuCo_all_NR_hilbert_fast_training_history_6_1.json')
    ...
    print('\n' + '=' * 80)
    print('ZuCo TRAINING COMPLETE (FAST) — All Subjects (v1 + v2) | NR')
